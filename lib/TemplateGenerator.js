...
class TemplateGenerator {
   ...
   */
  _create( options = {} ) {
    const {name, type, actions, postfix,filename} = options;
    ...
    if( options.isDir ) {
      ...
      const tpl = this._compileTpl(this._getSingleTpl(type), { name, actions, filesType,  postfix });
      this._createFile(name, type, filesType.script, tpl,filename, postfix);
    }
  }
  ...
  _compileTpl( file, { name, actions, filesType, postfix} ) {
    ...
    return compiled({ name: componentName, actions, filesType, postfix });
  }
   ...
   * @private
   */
  _createFile( name, type, fileType, tpl,filename, postfix ) {
    fs.outputFile(this._createFilePath(filename, type, fileType,postfix), tpl, function( err ) {
  ...
  _createDirectory( dirPath, data, fileTypes ) {
    fs.readdir(dirPath, ( err, dir ) => {
      const name = data.name + (data.postfix ? ' ' +data.postfix : '');
      ...
      dir.forEach(tempFile => {
          ...
          let fileName = this._createFileName(tempFile, data.name, fileTypes, 
            data.postfix);
  ...
  _createFileName( tempFile, name, fileTypes, postfix ) {
    let newName = tempFile.replace(/temp/, name + (postfix ? ('.' + postfix):''));
   ...
   * @private
   */
  _getSingleTpl( type , extension = 'tsx') {
   ...
   * @private
   */
  _createFilePath( name, type, fileType,postfix ) {
    name = name.replace(/ /g, '.');
    // adjust this logic and test
    if(type === 'hook' || type === 'store' || type === 'slice' || type === 'api'){
      let modFileName = `${name}.${fileType}`;
      return path.join(process.cwd(),this.path, modFileName );
    }
