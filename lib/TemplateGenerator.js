...
class TemplateGenerator {
  ...
  _create( options = {} ) {
    ...
    if( options.isDir ) {
      ...
      this._createFile(name, type, filesType.script, tpl,filename);
    }
  }
   ...
   * @private
   */
  _createFile( name, type, fileType, tpl,filename ) {
    fs.outputFile(this._createFilePath(filename, type, fileType), tpl, function( err ) {
  ...
  _createFilePath( name, type, fileType ) {
    name = name.replace(/ /g, '.');
    if(type === 'hook' || type === 'store' || type === 'slice'){
